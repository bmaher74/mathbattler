...
- "_thought_process": You MUST write this key first. Use it for scratchpad reasoning — rough arithmetic, crossed-out tries, or notes-to-self are OK here because this key is deleted before the student sees the question. Player-visible fields ("text"/"text_blocks", "expected_answer", "ideal_explanation", "success_criteria") must still agree with each other; never fix a broken stem only in scratchpad. You MUST plan the math before the story. Follow these headings in order inside this string:
  ...
  2) SOLUTION: Work toward a clean, verifiable answer (scratchpad may be messy). Put **every** intermediate numeric calculation and multi-step ledger here — **not** in the stem’s \`inline_math\` / \`\\(...\\)\`.
...
- **ANTI-SPOILER (STEM MATH):** Player-visible math (\`text\` inside \\(...\\), and every \`inline_math\` latex string) must **not** spell out a **chain of evaluated steps** (e.g. several clauses separated by commas/semicolons, each with \`=\` and a **numeric** result like \`4\\times 5=20,\\,20-4=16,\\ldots\`). That full scratch work belongs **only** in \`"_thought_process"\`. In the stem you may show **one** relation the task genuinely requires (e.g. a single equation to solve) or an **unevaluated** expression/template (e.g. \`4\\times 5\` or \`0.20\\times(\\text{subtotal})\` **without** \`= \\ldots\` answers). \`ideal_explanation\` may show the completed work after the battle.
...
- **NUMERIC INTEGRITY (CRITICAL):** Any number you mention in the story prose MUST be the exact same number used in the formal math you present (equation/expressions in \\(...\\) or "inline_math"). Do not introduce "flavor numbers" (e.g., 13 candles, 7 gargoyles) unless they are explicitly used in the math. If you revise the equation, you MUST revise every number in the prose to match (and vice versa). If you use units/currency, keep the units consistent with those same numbers in both prose and math.
